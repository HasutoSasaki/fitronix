...
1. **Given** セット完了後、**When** 「タイマー開始」ボタンをタップし、60秒を選択、**Then** タイマーがカウントダウンを開始
2. **Given** タイマー稼働中、**When** iPhoneをロック(スリープモード)、**Then** ロック画面にタイマーが通知として表示され、残り時間が更新される
3. **Given** ロック画面にタイマー表示中、**When** 通知の「一時停止」ボタンをタップ、**Then** タイマーが一時停止し、「再開」ボタンが表示される
4. **Given** タイマーが0秒に到達、**When** カウントダウン終了、**Then** バイブレーションと通知音が鳴り、「休憩終了」通知が表示される
...
- **タイマー実行中のアプリ終了**: ユーザーがアプリを強制終了した場合でも、通知は継続し、タイマー終了時に通知が届く
- **ロック画面でのタイマー操作**: 通知の一時停止・再開・スキップボタンが正常に動作し、アプリを開かずに制御できる
...
- **同じ種目名の重複登録**: ライブラリで同じ部位に同じ種目名を複数登録することは許可するが、警告を表示(例: 「既に同じ名前の種目が存在します」)
...
- **FR-008**: システムはすべてのデータを端末のローカルストレージに永続化する
...
- **FR-030**: ユーザーはいつでもデータをエクスポート可能(将来的な拡張として考慮)
...
- **SC-008**: ロック画面でのタイマー操作(一時停止・再開・スキップ)が100%の確率で正常に動作する
