...
- **同じ種目名の重複登録**: ライブラリで同じ部位に同じ種目名を複数登録することは**許可**する。登録前にUIが警告を表示(例: 「既に同じ名前の種目が存在します。続けますか?」)し、ユーザーが確認した場合のみ登録実行。これにより、バリエーション(例: バーベル・ベンチプレス vs ダンベル・ベンチプレス)の登録が可能
